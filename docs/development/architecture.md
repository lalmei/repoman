...
This page describes how the **create** command and **config validate** flow are structured: the CLI layer delegates to core modules `repoman.copier` (template and copy operations), `repoman.config` (project configuration and answers loading), `repoman.hotspots` (code hotspot analysis), and `repoman.compliance` (repository compliance analysis).
...
- **compliance**: The `repoman compliance` command group uses `repoman.compliance` to evaluate repositories against built-in readiness profiles, merge repo-declared evidence from `compliance.yml`, and render terminal, JSON, Markdown, or HTML reports.
