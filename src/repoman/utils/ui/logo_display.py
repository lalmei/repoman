...
from repoman.utils.ui.theme.theme import cool_ramp, set_theme
...
class BrailleCanvas:
    ...
    def render(self) -> list[str]:
        ...
        for row in range(self.term_height):
            ...
            line = "".join(chr(0x2800 + self._buf[offset + col]) for col in range(self.term_width))
            lines.append(line)
        return lines
                ...
                if cell == "#":
                    for sy in range(scale):
                        for sx in range(scale):
                            pixels.append((cursor_x + col_idx * scale + sx, row_idx * scale + sy))
...
def run_particle_logo(console: Console, hold_seconds: float = 1.5) -> None:
    ...
    style_final = theme.styles["lavender"]
    # Get target positions from text
    text_pixels_line1 = text_to_pixels("REPOsitory", scale=2)
    ...
    for line in canvas.render():
        for ch in line:
            if ch == chr(0x2800):
                ...
                final.append(ch, style=style_final)
        final.append("\n")
