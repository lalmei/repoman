...
from repoman.compliance.models import ControlResult, DetectorSpec


def evaluate_detector(repo_path: Path, detection: DetectorSpec) -> tuple[str, list[str]]:
...
def _file_exists(repo_path: Path, params: dict[str, Any]) -> tuple[str, list[str]]:
...
def _glob_exists(repo_path: Path, params: dict[str, Any]) -> tuple[str, list[str]]:
...
def _yaml_path_exists(repo_path: Path, params: dict[str, Any]) -> tuple[str, list[str]]:
    path = repo_path / str(params["path"])
    if not path.exists():
        return "unmet", [f"Missing {path.relative_to(repo_path)}"]
...
def _text_match(repo_path: Path, params: dict[str, Any]) -> tuple[str, list[str]]:
    path = repo_path / str(params["path"])
    if not path.exists():
        return "unmet", [f"Missing {path.relative_to(repo_path)}"]
...
def _github_actions_rule(repo_path: Path, params: dict[str, Any]) -> tuple[str, list[str]]:
...
def _make_target_exists(repo_path: Path, params: dict[str, Any]) -> tuple[str, list[str]]:
...
def _pyproject_field_exists(repo_path: Path, params: dict[str, Any]) -> tuple[str, list[str]]:
...
def _manual_only(_repo_path: Path, params: dict[str, Any]) -> tuple[str, list[str]]:
