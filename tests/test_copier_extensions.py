...
def _write_base_answers(project_dir: Path) -> Path:
    ...
    answers_file.write_text(
        yaml.safe_dump({"python_package_import_name": "pkg", "python_package_command_line_name": "pkg"}, sort_keys=False)
    )
    return answers_file
...
def test_create_command_extension_dry_run_does_not_write_manifest(tmp_path: Path) -> None:
    ...
    instance, copier_options, command_file, test_file = create_command_extension(
        ...
        answers={"python_package_import_name": "pkg", "python_package_command_line_name": "pkg"},
        command_name="foo",
        force=False,
        dry_run=True,
    )
...
def test_sync_extensions_runs_worker_update(tmp_path: Path) -> None:
    ...
    with patch("repoman.copier.extension_lifecycle.Worker", side_effect=[create_ctx, update_ctx]):
        create_command_extension(
            project_dir=tmp_path,
            base_answers_file=answers_file,
            answers={"python_package_import_name": "pkg", "python_package_command_line_name": "pkg"},
