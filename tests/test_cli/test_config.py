...
def test_config_init_writes_file(
    ...
    result = cli_runner.invoke(
        cli_app, ["config", "init", "--output", str(out)], input=""
    )
    assert result.exit_code == 0
    assert out.exists()
...
def test_config_init_respects_output_option(
    cli_runner: CliRunner, cli_app: Typer, tmp_path: Path
) -> None:
...
def test_config_init_overwrites_with_force(
    ...
    out.write_text("existing content")
    result = cli_runner.invoke(
        cli_app, ["config", "init", "--output", str(out), "--force"], input=""
    )
    assert result.exit_code == 0
    content = out.read_text(encoding="utf-8")
...
def test_config_init_dry_run_does_not_write(
    cli_runner: CliRunner, cli_app: Typer, tmp_path: Path
) -> None:
    """Test config init with --dry-run does not create file."""
    out = tmp_path / "would-be-created.yml"
    result = cli_runner.invoke(
        cli_app, ["--dry-run", "config", "init", "--output", str(out)], input=""
    )
    assert result.exit_code == 0
...
def test_config_init_output_directory_resolves_to_copier_answers(
    ...
    result = cli_runner.invoke(
        cli_app, ["config", "init", "--output", str(tmp_path)], input=""
    )
    assert result.exit_code == 0
...
def test_config_init_custom_template(
    cli_runner: CliRunner, cli_app: Typer, tmp_path: Path
) -> None:
...
def test_config_init_template_not_found(
    cli_runner: CliRunner, cli_app: Typer, tmp_path: Path
) -> None:
