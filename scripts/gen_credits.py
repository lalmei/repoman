...
def _get_deps(base_deps: dict[str, Requirement], metadata: Metadata) -> Metadata:
    ...
    for dep_name, dep_req in base_deps.items():
        if (
            dep_name not in metadata
            or dep_name == "{{ python_package_distribution_name }}"
        ):
            continue
...
def _render_credits() -> str:
    ...
    template_text = dedent(
        ...
        These projects were third-party software used to build *{{ project_name }}*. 
        
        SOUP Documentation for loopr_ml:
